...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-[#F0E6D8]/90 backdrop-blur-md border-b border-[#D4A036]/20 shadow-soft transition-all duration-300 ${
        ...
        <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-1">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 hover:bg-[#D4A036]/10 hover:text-[#D4A036] ${
                    activeSection === item.href.slice(1)
                      ? 'text-[#D4A036] bg-[#D4A036]/5'
                      : 'text-[#3D3228]'
                  }`}
                >
                  {item.name}
          ...
          <div className="md:hidden">
            <Button
              ...
              className="text-[#3D3228] hover:bg-[#D4A036]/10"
            >
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 bg-[#F0E6D8]/90 backdrop-blur-md border-b border-[#D4A036]/20">
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className={`block w-full text-left px-3 py-2 text-base font-medium rounded-md transition-all duration-300 hover:bg-[#D4A036]/10 hover:text-[#D4A036] ${
                  activeSection === item.href.slice(1)
                    ? 'text-[#D4A036] bg-[#D4A036]/5'
                    : 'text-[#3D3228]'
                }`}
